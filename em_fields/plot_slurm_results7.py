...
for set_ind in range(len(set_names)):
    ...
    # ind_points = range(10)
    # ind_points = range(20)
    # ind_points = range(0, 100, 5)
    # ind_points = range(100, 200, 5)
    # ind_points = range(200, 300, 5)
    # ind_points = range(300, 400, 5)
    ...
    for ind_point in ind_points:
        ...
        if abs(initial_angle - loss_cone_angle) < 1000:
            ...
            if plot_z_v:
                ...
                # if particle_type == 'right':
                plt.figure(3)
                plt.plot(v_transverse / settings['v_th'], z / settings['l'], '-', linewidth=1, alpha=0.5)

    if plot_z_v:
        plt.plot(v_transverse[0] / settings['v_th'], z[0] / settings['l'], 'ok')
        # add the plot labels
        # plt.figure(3)
        plt.ylabel('$z/l$')
