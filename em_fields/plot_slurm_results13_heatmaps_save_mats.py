                    ...
                    if os.path.exists(compiled_save_file):
                        ...
                        for ind_beta, beta in enumerate(beta_loop_list):
                            for ind_alpha, alpha in enumerate(alpha_loop_list):
                                ...
                                N_rc[ind_beta, ind_alpha] = np.mean(particles_counter_mat_3d[0, 1, inds_t_saturation])
                                N_lc[ind_beta, ind_alpha] = np.mean(particles_counter_mat_3d[2, 1, inds_t_saturation])
                                N_cr[ind_beta, ind_alpha] = np.mean(particles_counter_mat_3d[1, 0, inds_t_saturation])
                                N_cl[ind_beta, ind_alpha] = np.mean(particles_counter_mat_3d[1, 2, inds_t_saturation])
                                N_rl[ind_beta, ind_alpha] = np.mean(particles_counter_mat_3d[0, 2, inds_t_saturation])
                                N_lr[ind_beta, ind_alpha] = np.mean(particles_counter_mat_3d[2, 0, inds_t_saturation])
                                percent_ok[ind_beta, ind_alpha] = percent_ok_curr
                                E_ratio_mean[ind_beta, ind_alpha] = (np.mean(data_dict['v'][inds_particles, -1] ** 2)
